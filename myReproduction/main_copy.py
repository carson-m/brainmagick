...
def train_net(device, train_loader, validation_loader, test_loader, audio_embeddings, train_set_size, validation_set_size, n_subjects, lr, num_epochs, load_model=False, model_to_load=None):
    ...
    # return best_model, best_epoch, train_loss_array, validation_loss_array, top10_acc_train_array, top10_acc_validation_array
    return train_loss_array, validation_loss_array, top10_acc_train_array, top10_acc_validation_array
...
def __main__():
    ...
    train_audio_embeddings_source = audio_embeddings[~test_set_mask]
    train_segment_label_source = segment_label[~test_set_mask]
    ...
    for i in range(cross_validation):
        ...
        for data_file in data_file_list:
            ...
            train_audio_embeddings = torch.cat((train_audio_embeddings, torch.tensor(train_audio_embeddings_source[train_epoch_mask])), dim = 0)
            train_segment_label = np.append(train_segment_label, train_segment_label_source[train_epoch_mask])
            ...
            validation_audio_embeddings = torch.cat((validation_audio_embeddings, torch.tensor(train_audio_embeddings_source[validation_epoch_mask])), dim = 0)
            validation_channel_layout = torch.cat((validation_channel_layout, channel_layout[validation_epoch_mask]), dim = 0)
            validation_segment_label = np.append(validation_segment_label, train_segment_label_source[validation_epoch_mask])
        ...
        # Train Net
        train_loss_array, validation_loss_array, top10_train_acc, top10_validation_acc = train_net(device, train_loader, validation_loader, test_loader, audio_embeddings, train_brain_segments.shape[0], validation_brain_segments.shape[0], num_subject, lr=lr, num_epochs=num_epochs)
        
        # # To see if the net was properly saved
        # print('Testing if the net is properly saved, testing with validation set')
        # top10_test_acc = test_net(device, validation_loader, audio_embeddings, num_subject, mynet)
        ...
        sio.savemat(save_location_tmp + 'result.mat', {'train_loss': train_loss_array, 'validation_loss': validation_loss_array, 'top10_train_acc': top10_train_acc, 'top10_validation_acc': top10_validation_acc})
        print('Result saved')
