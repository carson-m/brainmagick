...
def test_net(device, test_loader, audio_embeddings, n_subjects, model_to_load):
    ...
    for data, channel_layout, subjects, ground_truth, segment_label in test_loader:
        ...
        top10_acc_test += eval.get_correct(output.detach(), segment_label.detach(), 10).item()
...
def train_net(device, train_loader, test_loader, audio_embeddings, train_set_size, test_set_size, n_subjects, lr, num_epochs, load_model=False, model_to_load=None):
    ...
    for epoch in range(num_epochs):
        ...
        for data, channel_layout, subjects, ground_truth, segment_label in train_loader:
            ...
            top10_acc_train += eval.get_correct(output.detach(), segment_label.detach(), 10).item()
        ...
        print('Epoch:', epoch, 'Train Loss:', train_loss, 'Validation Loss:', test_loss, 'Top-10 Train Acc:', top10_acc_train, 'Top-10 Validation Acc:', top10_acc_test)
...
def __main__():
    ...
    for i in range(cross_validation):
        ...
        validation_loader = DataLoader(validation_dataset, batch_size=32, shuffle=True, num_workers=num_workers)
        mynet, train_loss_array, validation_loss_array, top10_train_acc, top10_validation_acc = train_net(device, train_loader, validation_loader, audio_embeddings, train_brain_segments.shape[0], validation_brain_segments.shape[0], num_subject, lr=lr, num_epochs=num_epochs)
        ...
        sio.savemat(save_location_tmp + 'result.mat', {'train_loss': train_loss_array, 'validation_loss': validation_loss_array, 'top10_train_acc': top10_train_acc, 'top10_validation_acc': top10_validation_acc, 'top10_test_acc': top10_test_acc})
        print('Result saved')
